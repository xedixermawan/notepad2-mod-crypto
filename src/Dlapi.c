...
BOOL DirList_StartIconThread(HWND hwnd)
{
    ...
    HANDLE ictHandle = CreateThread(NULL, 0, DirList_IconThread, (LPVOID)lpdl, 0, &dwtid);

    return ((ictHandle != NULL) && (ictHandle != INVALID_HANDLE_VALUE)) ? TRUE : FALSE;
}
...
//
BOOL DirList_GetLongPathName(HWND hwnd, LPWSTR lpszLongPath, DWORD bufLen)
{
    WCHAR tch[MAX_PATH];
    LPDLDATA lpdl = (LPVOID)GetProp(hwnd, pDirListProp);
...
#define LVIS_FLAGS LVIS_SELECTED|LVIS_FOCUSED
    ...
    if (!lpszFullPath || !StringLength(lpszFullPath))
        return(FALSE);
    else
        GetShortPathName(lpszFullPath, szShortPath, MAX_PATH);

    if (!lpszDisplayName || !StringLength(lpszDisplayName))
    {
        ZeroMemory(&shfi, sizeof(SHFILEINFO));
                          ...
                          BOOL bExcludeFilter)
{
    ...
    if (!lpszFileSpec || !lstrcmp(lpszFileSpec, L"*.*") || !StringLength(lpszFileSpec))
        return;
