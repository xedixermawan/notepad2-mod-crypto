...
BOOL OpenWithDlg(HWND hwnd, LPDLITEM lpdliParam)
{
    ...
    if (IDOK == ThemedDialogBoxParam(g_hInstance, MAKEINTRESOURCE(IDD_OPENWITH),
        ...
        if (PathIsLnkToDirectory(dliOpenWith.szFileName, szDestination,
            ...
            GetShortPathName(szParam, szParam, MAX_PATH);
        ...
        case IDCANCEL:
            if (bHasCapture) {
                ...
                if (LOWORD(wParam) == IDOK) {
                    ...
                    if (GetDlgItemText(hwnd, IDC_WINCLASS, tch, COUNTOF(tch)))
                        lstrcpyn(pTargetWndClassBuf, tch, MAX_PATH + 4);
                }
...
INT_PTR CALLBACK FindTargetDlgProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        #if defined(_WIN64)
            lstrcpy(szTargetApplication, L"Notepad2Cryptx64.exe");
        #else
            lstrcpy(szTargetApplication, L"Notepad2Crypt.exe");
        #endif
            lstrcpy(szTargetApplicationParams, L"/rs");
            lstrcpy(szTargetApplicationWndClass, L"Notepad2Crypt");
            lstrcpy(szDDEMsg, L"");
            lstrcpy(szDDEApp, L"");
